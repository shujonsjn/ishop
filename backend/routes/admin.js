...
router.get('/dashboard', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const topUsers = await db.prepare(
      "SELECT u.name, u.email, COUNT(o.id) AS order_count, COALESCE(SUM(o.total),0) AS total_spent FROM orders o JOIN users u ON u.id = o.user_id WHERE 1=1" + dateFilter + " GROUP BY o.user_id ORDER BY total_spent DESC LIMIT 5"
    ).all(...dateParams);
...
router.post('/products', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { name, en_name, description, price, purchase_price, compare_price, category_id, stock, featured, active, images, colors, brand, sku, has_sizes, sizes, color_images, size_chart_image, is_fast, is_verified } = req.body || {};
    if (!name || !price) return res.status(400).json({ error: 'name and price are required' });
    const slug = name.toLowerCase().replace(/[^\w\u0980-\u09FF]+/g, '-').replace(/^-+|-+$/g, '') || 'product';
    const result = await db.prepare('INSERT INTO products (name, en_name, slug, description, price, purchase_price, compare_price, category_id, stock, featured, active, images, colors, brand, sku, has_sizes, sizes, color_images, size_chart_image, is_fast, is_verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(name, en_name || '', slug, description || '', Number(price), Number(purchase_price) || 0, compare_price ? Number(compare_price) : null, category_id || null, stock || 0, featured ? 1 : 0, active !== 0 ? 1 : 0, JSON.stringify(images || []), JSON.stringify(colors || []), brand || '', sku || '', has_sizes ? 1 : 0, JSON.stringify(sizes || []), JSON.stringify(color_images || {}), size_chart_image || '', is_fast ? 1 : 0, is_verified ? 1 : 0);
